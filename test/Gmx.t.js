...
describe("GMXFactory", async function () {
  ...
  before(async function () {
    ...
    positionKeeper = await PositionKeeper.attach(
      process.env.TESTNET_POSITION_KEEPER
    );
    ...
    _acceptablePriceLongETH = _acceptablePriceETH
      .mul(percentageLongETH)
      .div(ethers.utils.parseUnits("1", 4));
    _acceptablePriceShortETH = _acceptablePriceETH
      .mul(percentageShortETH)
      .div(ethers.utils.parseUnits("1", 4));

    //Get the Current BTC Price from the Vault in 1e30
    const _acceptablePriceBTC = await vault.getMaxPrice(
    ...
    _acceptablePriceLongBTC = _acceptablePriceBTC
      .mul(percentageLongBTC)
      .div(ethers.utils.parseUnits("1", 4));
    _acceptablePriceShortBTC = _acceptablePriceBTC
      .mul(percentageShortBTC)
      .div(ethers.utils.parseUnits("1", 4));
    ...
    const usdAmount = ethAmount
      .mul(_acceptablePriceETH1e18)
    ...
    const sizeDeltaDeduction = _sizeDeltaOld
      .mul(deductionPercentage)
      .div(ethers.utils.parseUnits("1", 3));
  ...
  it("should create a long positions for USDT/ETH", async function () {
    ...
    approvalTx = await tokenUSDT
      .connect(deployer)
      .approve(gmxFactory.address, amountIn);
    await approvalTx.wait(3); // Wait for 3 blocks confirmations

    const creatingPositionTx = await gmxFactory
      .connect(deployer)
      .openLongPosition(
  ...
  it("should verify the position of USDT/ETH we create", async function () {
    ...
    // Get the number of positions for the deployer
    const positionId = await gmxFactory.indexedPositions(
      deployer.address,
      positions
    );
  ...
  it("should close a Long position of USDT/ETH we created", async function () {
    ...
    const closeTx = await gmxFactory.closePosition(
      positionId,
      [process.env.TESTNET_WETH],
      _acceptablePriceLongETH,
      true,
