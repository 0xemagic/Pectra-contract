...
contract GMXFactory {
    ...
    mapping(bytes32 => mapping(address => PositionStatus)) public positionDetails;
    ...
    // Events
    event TokensWithdrawn(address indexed token, address indexed to, uint256 indexed amount);
    ...
    event PlatformLogicsFactoryChanged(address indexed factory, bool indexed newState);

    event PlatformLogicChanged(IPlatformLogic oldAddress, IPlatformLogic newAddress);
    ...
    function setAllowedToken(IERC20 _tokenFeePaymentAddress, bool _allowed) external onlyOwner {
    ...
    function openLongPosition(
    ...
    ) public payable onlyAllowedTokens(IERC20(_path[0])) returns (bytes32 positionId) {
        ...
        uint256 _amount = PLATFORM_LOGIC.applyPlatformFeeErc20(msg.sender, _amountIn, IERC20(collateral), address(this));
    ...
    function openLongPositionEth(
        ...
        uint256 _value = PLATFORM_LOGIC.calculateFees(msg.value, PLATFORM_LOGIC.viewPlatformFee());

        /// @dev apply platform fees with eth
        /// @notice _amount = the amount left after the apply of platformFees
        uint256 _amount = PLATFORM_LOGIC.applyPlatformFeeEth{value: _value}(msg.sender, msg.value);

        positionId = IGMXAdapter(adapter).createIncreasePositionETH{value: _amount}(
     ...
     */
    function openPositions(nftData memory _nftData) external payable returns (bytes32, bytes32) {
    ...
    function createDecreasePosition(
        ...
        require(positionDetails[_positionId][msg.sender] == PositionStatus.Opened, "GMX FACTORY: Position not open");
        address adapter = positionAdapters[_positionId];
        positionId = IGMXAdapter(adapter).createDecreasePosition{value: msg.value}(
     ...
     * @param _withdrawETH Whether to withdraw ETH after closing the position.
     */
    function closePosition(bytes32 _positionId, address[] memory _path, uint256 _acceptablePrice, bool _withdrawETH)
        external
        payable
    {
        require(msg.sender == positionOwners[_positionId], "GMX FACTORY: Not a position owner");
     ...
     */
    function getPositionStatus(bytes32 _positionId, address _address) external view returns (PositionStatus) {
     ...
     */
    function getTotalPositions(address _address) external view returns (uint256) {
     ...
     */
    function getPositionId(address _address, uint256 _index) external view returns (bytes32) {
     ...
     */
    function updateOwner(address _oldOwner, address _newOwner, bytes32 _positionId)
        external
        onlyNftHandler
        returns (bool)
    {
    ...
    function setPlatformLogic(IPlatformLogic _platformLogic) external onlyOwner {
    ...
    function changePectraTreasury(address payable _newTreasury) external onlyOwner {
        PLATFORM_LOGIC.changePectraTreasury(_newTreasury);
    }

    /// @notice changes the spectra staking contract address
    function changePectraStakingContract(address payable _newStakingContract) external onlyOwner {
    ...
    function editReferralCode(address _referee, bytes32 _referralCode) external onlyOwner {
        PLATFORM_LOGIC.editReferralCode(_referee, _referralCode);
    }

    /// @notice used so admins can edit users who are referred
    function editReferredUsers(address _referrer, bytes32 _referralCode) external onlyOwner {
        PLATFORM_LOGIC.editReferredUsers(_referrer, _referralCode);
    }

    function tokenTransferPlatformLogic(IERC20 _token, address _from, address _to, uint256 _amount)
        external
