...
contract GMXAdapter is Initializable, IPositionRouterCallbackReceiver {
    ...
    // Modifier to restrict access to only the contract owner or factory contract or positionRouter
    modifier onlyOwnerOrPositionRouter() {
        require(
            OWNER == msg.sender || FACTORY == msg.sender,
            POSITION_ROUTER == msg.sender,
            "GMX ADAPTER: caller is not the owner or factory or gmx position router"
        );
        _;
    }
     ...
     * @param _receiver The address to which the collateral will be transferred after closing the position.
     */
    function closeFailedPosition(address[] memory _path, address _receiver) public payable onlyOwnerOrPositionRouter {
