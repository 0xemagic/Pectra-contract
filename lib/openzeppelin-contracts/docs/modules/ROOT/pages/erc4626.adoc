...
When plotted in log-log scale, the rate is defined similarly, but apspectras differently (because the point (0,0) is infinitely far away). Rates are represented by "diagonal" lines with different offsets.
