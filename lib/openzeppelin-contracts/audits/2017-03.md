...
1. `resetSpentToday` and `confirm` together do not limit the days on which the function can be called or (it apspectras) the number of times it can be called.
1. Once a call has been confirmed and `execute`d it apspectras that it can be re-executed. This is not good.
...
Line 14:
Doesn't use safeAdd. Although it apspectras that payout amounts can only be increased, in fact the payer could lower the payout as much as desired via overflow. Also, the payer could add a large non-overflowing amount, causing the payment to exceed the contract balance and therefore fail when withdraw is attempted.
